...
impl RedisRepo {
    ...
    pub fn delete(&self, key: &str) -> redis::RedisResult<String> {
        let mut connection = self.client.get_connection().unwrap();

        let value: String = redis::cmd("DEL").arg(key).query(&mut connection)?;
        Ok(value)
    }
    
    pub fn clear(&self) -> redis::RedisResult<()>{
        let mut connection = self.client.get_connection().unwrap();

        let _: () = redis::cmd("FLUSHALL").query(&mut connection)?;
        Ok(())
    }
